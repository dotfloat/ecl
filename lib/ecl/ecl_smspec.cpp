...
const ecl::smspec_node *ecl_smspec_get_var_node(const node_map &mp,
                                                const char *var) {
...
ecl_smspec_get_str_key_var_node(const std::map<std::string, node_map> &mp,
...
const ecl::smspec_node *
ecl_smspec_get_int_key_var_node(const std::map<int, node_map> &mp, int key,
                                const char *var) {
    const auto key_it = mp.find(key);
    if (key_it == mp.end())
        return nullptr;
...
int *ecl_smspec_alloc_mapping(const ecl_smspec_type *self,
    ...
    for (int i = 0; i < ecl_smspec_num_nodes(self); i++) {
        const ecl::smspec_node &self_node =
            ecl_smspec_iget_node_w_node_index(self, i);
        int self_index = self_node.get_params_index();
        const char *key = self_node.get_gen_key1();
        if (ecl_smspec_has_general_var(other, key)) {
            const ecl::smspec_node &other_node =
                ecl_smspec_get_general_var_node(other, key);
...
const ecl::smspec_node &
ecl_smspec_iget_node_w_node_index(const ecl_smspec_type *smspec,
                                  int node_index) {
...
const ecl::smspec_node &ecl_smspec_iget_node(const ecl_smspec_type *smspec,
                                             int index) {
    return ecl_smspec_iget_node_w_node_index(smspec, index);
}

const ecl::smspec_node &
ecl_smspec_iget_node_w_params_index(const ecl_smspec_type *smspec,
                                    int params_index) {
...
static void ecl_smspec_fwrite_INTEHEAD(const ecl_smspec_type *smspec,
                                       fortio_type *fortio) {
    ecl_kw_type *intehead =
        ecl_kw_alloc(INTEHEAD_KW, INTEHEAD_SMSPEC_SIZE, ECL_INT);
    ecl_kw_iset_int(intehead, INTEHEAD_SMSPEC_UNIT_INDEX, smspec->unit_system);
    /*
    The simulator type is just hardcoded to ECLIPSE100.
  */
    ecl_kw_iset_int(intehead, INTEHEAD_SMSPEC_IPROG_INDEX,
                    INTEHEAD_ECLIPSE100_VALUE);
    ecl_kw_fwrite(intehead, fortio);
    ecl_kw_free(intehead);
}

static void ecl_smspec_fwrite_RESTART(const ecl_smspec_type *smspec,
                                      fortio_type *fortio) {
    ecl_kw_type *restart_kw =
        ecl_kw_alloc(RESTART_KW, SUMMARY_RESTART_SIZE, ECL_CHAR);
...
static void ecl_smspec_fwrite_DIMENS(const ecl_smspec_type *smspec,
                                     fortio_type *fortio) {
    ...
    ecl_kw_iset_int(dimens_kw, DIMENS_SMSPEC_RESTART_STEP_INDEX,
                    smspec->restart_step);
...
static void ecl_smspec_fwrite_STARTDAT(const ecl_smspec_type *smspec,
                                       fortio_type *fortio) {
    ecl_kw_type *startdat_kw =
        ecl_kw_alloc(STARTDAT_KW, STARTDAT_SIZE, ECL_INT);
    int second, minute, hour, mday, month, year;
    ecl_util_set_datetime_values(smspec->sim_start_time, &second, &minute,
...
static void ecl_smspec_fortio_fwrite(const ecl_smspec_type *smspec,
                                     fortio_type *fortio) {
    ...
    ecl_kw_type *wgnames_kw =
        ecl_kw_alloc(ecl_type_is_char(names_type) ? WGNAMES_KW : NAMES_KW,
    ...
    */
        if (smspec_node.get_var_type() == ECL_SMSPEC_INVALID_VAR) {
            ...
            ecl_kw_iset_string8(keywords_kw, i,
...
void ecl_smspec_fwrite(const ecl_smspec_type *smspec, const char *ecl_case,
                       bool fmt_file) {
    char *filename = ecl_util_alloc_filename(
        NULL, ecl_case, ECL_SUMMARY_HEADER_FILE, fmt_file, 0);
    fortio_type *fortio =
        fortio_open_writer(filename, fmt_file, ECL_ENDIAN_FLIP);
...
static ecl_smspec_type *
ecl_smspec_alloc_writer__(const char *key_join_string, const char *restart_case,
  ...
  */
    if (restart_case) {
        if (strlen(restart_case) <=
            (SUMMARY_RESTART_SIZE * ECL_STRING8_LENGTH)) {
...
ecl_smspec_type *ecl_smspec_alloc_restart_writer(
    ...
    return ecl_smspec_alloc_writer__(key_join_string, restart_case,
                                     restart_step, sim_start, time_in_days, nx,
                                     ny, nz);
}
...
ecl_smspec_type *ecl_smspec_alloc_writer(const char *key_join_string,
                                         ...
                                         int nx, int ny, int nz) {
    return ecl_smspec_alloc_writer__(key_join_string, NULL, 0, sim_start,
...
static int ecl_smspec_get_global_grid_index(const ecl_smspec_type *smspec,
                                            int i, int j, int k) {
...
static void ecl_smspec_install_gen_keys(ecl_smspec_type *smspec,
                                        const ecl::smspec_node &smspec_node) {
...
static void
ecl_smspec_install_special_keys(ecl_smspec_type *ecl_smspec,
                                const ecl::smspec_node &smspec_node) {
    /**
  ...
  */
    ...
    switch (var_type) {
    case (ECL_SMSPEC_COMPLETION_VAR):
        ecl_smspec->well_completion_var_index[well][num][keyword] =
            &smspec_node;
        break;
    case (ECL_SMSPEC_FIELD_VAR):
...
static void ecl_smspec_load_restart(ecl_smspec_type *ecl_smspec,
                                    const ecl_file_type *header) {
    if (ecl_file_has_kw(header, RESTART_KW)) {
        const ecl_kw_type *restart_kw =
            ecl_file_iget_named_kw(header, RESTART_KW, 0);
        char tmp_base
      ...
      */
            ...
            std::string path =
                ecl::util::path::dirname(ecl_smspec->header_file);
            smspec_header = ecl_util_alloc_exfilename(
                path.c_str(), restart_base, ECL_SUMMARY_HEADER_FILE,
                ecl_smspec->formatted, 0);
            if (smspec_header) {
                if (!util_same_file(
                        smspec_header,
                        ecl_smspec->header_file
...
static const ecl::smspec_node *
ecl_smspec_insert_node(ecl_smspec_type *ecl_smspec,
                       std::unique_ptr<ecl::smspec_node> smspec_node) {
    ...
    ecl_smspec->inv_index_map.insert(
        std::make_pair(params_index, ecl_smspec->smspec_nodes.size()));
    ...
    if (static_cast<int>(ecl_smspec->smspec_nodes.size()) >
        ecl_smspec->params_size)
...
const ecl::smspec_node *ecl_smspec_add_node(ecl_smspec_type *ecl_smspec,
    ...
    return ecl_smspec_insert_node(
        ecl_smspec, std::unique_ptr<ecl::smspec_node>(new ecl::smspec_node(
                        params_index, keyword, num, unit, ecl_smspec->grid_dims,
                        default_value, ecl_smspec->key_join_string.c_str())));
}
...
const ecl::smspec_node *ecl_smspec_add_node(ecl_smspec_type *ecl_smspec,
    ...
    int params_index = ecl_smspec->smspec_nodes.size();
    return ecl_smspec_insert_node(
        ecl_smspec, std::unique_ptr<ecl::smspec_node>(
                        new ecl::smspec_node(node, params_index)));
}
...
const ecl::smspec_node *
ecl_smspec_add_node(ecl_smspec_type *ecl_smspec, const char *keyword,
                    const char *wgname, const char *unit, float default_value) {
    int params_index = ecl_smspec->smspec_nodes.size();
    return ecl_smspec_insert_node(
        ecl_smspec, std::unique_ptr<ecl::smspec_node>(new ecl::smspec_node(
...
const ecl::smspec_node *ecl_smspec_add_node(ecl_smspec_type *ecl_smspec,
    ...
    return ecl_smspec_insert_node(
        ecl_smspec,
        std::unique_ptr<ecl::smspec_node>(new ecl::smspec_node(
            params_index, keyword, wgname, num, unit, ecl_smspec->grid_dims,
            default_value, ecl_smspec->key_join_string.c_str())));
}

const ecl::smspec_node *
ecl_smspec_add_node(ecl_smspec_type *ecl_smspec, int params_index,
 ...
 * support.
 */
static const char *get_active_keyword_alias(ecl_file_type *header,
                                            const char *keyword) {
...
static bool ecl_smspec_check_header(ecl_file_type *header) {
    ...
    for (size_t i = 0; i < num_req_keywords && OK; i++) {
        OK &= ecl_file_has_kw(header, get_active_keyword_alias(
...
static bool ecl_smspec_fread_header(ecl_smspec_type *ecl_smspec,
    ...
    if (header && ecl_smspec_check_header(header)) {
        ...
        if (ecl_file_has_kw(header, INTEHEAD_KW)) {
            const ecl_kw_type *intehead =
                ecl_file_iget_named_kw(header, INTEHEAD_KW, 0);
            ecl_smspec->unit_system = (ert_ecl_unit_enum)ecl_kw_iget_int(
      ...
      */
...

        {
            ...
            ecl_smspec->sim_start_time =
                ecl_util_make_datetime(sec, min, hour, day, month, year);
        }

        ecl_smspec->grid_dims[0] =
            ecl_kw_iget_int(dimens, DIMENS_SMSPEC_NX_INDEX);
        ecl_smspec->grid_dims[1] =
            ecl_kw_iget_int(dimens, DIMENS_SMSPEC_NY_INDEX);
        ecl_smspec->grid_dims[2] =
            ecl_kw_iget_int(dimens, DIMENS_SMSPEC_NZ_INDEX);
        ecl_smspec->restart_step =
            ecl_kw_iget_int(dimens, DIMENS_SMSPEC_RESTART_STEP_INDEX);

        ecl_util_get_file_type(header_file, &ecl_smspec->formatted, NULL);

        {
            for (params_index = 0; params_index < ecl_kw_get_size(wells);
                 params_index++) {
                float default_value = PARAMS_GLOBAL_DEFAULT;
                int num = SMSPEC_NUMS_INVALID;
                char *well = (char *)util_alloc_strip_copy(
                    (const char *)ecl_kw_iget_ptr(wells, params_index));
                char *kw = (char *)util_alloc_strip_copy(
                    (const char *)ecl_kw_iget_ptr(keywords, params_index));
                char *unit = (char *)util_alloc_strip_copy(
                    (const char *)ecl_kw_iget_ptr(units, params_index));

                ecl_smspec_var_type var_type;
                if (nums != NULL)
                    num = ecl_kw_iget_int(nums, params_index);
                ...
                if (ecl_smspec_lgr_var_type(var_type)) {
                    ...
                    char *lgr_name = (char *)util_alloc_strip_copy(
                        (const char *)ecl_kw_iget_ptr(lgrs, params_index));

                    ecl_smspec_insert_node(
                        ecl_smspec,
                        std::unique_ptr<ecl::smspec_node>(new ecl::smspec_node(
                    ...
                    ecl_smspec_insert_node(
                        ...
                        std::unique_ptr<ecl::smspec_node>(new ecl::smspec_node(
                            ...
                            ecl_smspec->grid_dims, default_value,
                            ecl_smspec->key_join_string.c_str())));
...
ecl_smspec_type *ecl_smspec_fread_alloc(const char *header_file,
    ...
    if (ecl_smspec_fread_header(ecl_smspec, header_file, include_restart)) {
        ...
        const ecl::smspec_node *time_node =
            ecl_smspec_get_var_node(ecl_smspec->misc_var_index, "TIME");
        if (time_node) {
        ...
        const ecl::smspec_node *day_node =
            ecl_smspec_get_var_node(ecl_smspec->misc_var_index, "DAY");
        if (day_node != NULL) {
            ecl_smspec->day_index = smspec_node_get_params_index(day_node);
            ecl_smspec->month_index = smspec_node_get_params_index(
                &ecl_smspec->misc_var_index["MONTH"]);
            ecl_smspec->year_index = smspec_node_get_params_index(
                &ecl_smspec->misc_var_index["YEAR"]);
        }
...
ecl_smspec_get_well_var_node(const ecl_smspec_type *smspec, const char *well,
                             const char *var) {
    const auto node_ptr =
        ecl_smspec_get_str_key_var_node(smspec->well_var_index, well, var);
    if (!node_ptr)
...
int ecl_smspec_get_well_var_params_index(const ecl_smspec_type *ecl_smspec,
    ...
    const auto node_ptr =
        ecl_smspec_get_str_key_var_node(ecl_smspec->well_var_index, well, var);
...
ecl_smspec_get_group_var_node(const ecl_smspec_type *smspec, const char *group,
                              const char *var) {
    const auto node_ptr =
        ecl_smspec_get_str_key_var_node(smspec->group_var_index, group, var);
    if (!node_ptr)
...
int ecl_smspec_get_group_var_params_index(const ecl_smspec_type *ecl_smspec,
    ...
    const auto node_ptr = ecl_smspec_get_str_key_var_node(
        ecl_smspec->group_var_index, group, var);
    return node_valid_index(node_ptr);
}
...
const ecl::smspec_node &
ecl_smspec_get_block_var_node(const ecl_smspec_type *ecl_smspec,
                              const char *block_var, int block_nr) {
    const auto node_ptr = ecl_smspec_get_int_key_var_node(
        ecl_smspec->block_var_index, block_nr, block_var);
    if (!node_ptr)
...
const ecl::smspec_node &
ecl_smspec_get_block_var_node_ijk(const ecl_smspec_type *ecl_smspec,
                                  const char *block_var, int i, int j, int k) {
    return ecl_smspec_get_block_var_node(
...
bool ecl_smspec_has_block_var_ijk(const ecl_smspec_type *ecl_smspec,
                                  const char *block_var, int i, int j, int k) {
    return ecl_smspec_has_block_var(
        ecl_smspec, block_var,
        ecl_smspec_get_global_grid_index(ecl_smspec, i, j, k));
}
...
int ecl_smspec_get_block_var_params_index_ijk(const ecl_smspec_type *ecl_smspec,
                                              ...
                                              int j, int k) {
    return ecl_smspec_get_block_var_params_index(
        ecl_smspec, block_var,
        ecl_smspec_get_global_grid_index(ecl_smspec, i, j, k));
}
...
const ecl::smspec_node &
ecl_smspec_get_region_var_node(const ecl_smspec_type *ecl_smspec,
                               const char *region_var, int region_nr) {
    const auto node_ptr = ecl_smspec_get_int_key_var_node(
        ecl_smspec->region_var_index, region_nr, region_var);
    if (!node_ptr)
        throw std::out_of_range("No such block variable");
...
const ecl::smspec_node &
ecl_smspec_get_well_completion_var_node(const ecl_smspec_type *ecl_smspec,
                                        const char *well, const char *var,
                                        int cell_nr) {
    const auto node_ptr = ecl_smspec_get_well_completion_var_node__(
        ecl_smspec, well, var, cell_nr);
    if (!node_ptr)
...
int ecl_smspec_get_well_completion_var_params_index(
    const ecl_smspec_type *ecl_smspec, const char *well, const char *var,
    int cell_nr) {
    const auto node_ptr = ecl_smspec_get_well_completion_var_node__(
        ecl_smspec, well, var, cell_nr);
    return node_valid_index(node_ptr);
...
const ecl::smspec_node &
ecl_smspec_get_general_var_node(const ecl_smspec_type *smspec,
                                const char *lookup_kw) {
    const auto node_ptr =
        ecl_smspec_get_var_node(smspec->gen_var_index, lookup_kw);
    if (!node_ptr)
...
int ecl_smspec_get_general_var_params_index(const ecl_smspec_type *ecl_smspec,
                                            const char *lookup_kw) {
    const auto node_ptr =
        ecl_smspec_get_var_node(ecl_smspec->gen_var_index, lookup_kw);
    return node_valid_index(node_ptr);
}
...
ecl_smspec_get_params_default(const ecl_smspec_type *ecl_smspec) {
    return ecl_smspec->params_default;
}

void ecl_smspec_free(ecl_smspec_type *ecl_smspec) { delete ecl_smspec; }
...
bool ecl_smspec_general_is_total(const ecl_smspec_type *smspec,
                                 const char *gen_key) {
    const ecl::smspec_node &smspec_node =
...
stringlist_type *
ecl_smspec_alloc_matching_general_var_list(const ecl_smspec_type *smspec,
                                           const char *pattern) {
...
ecl_smspec_alloc_map_list(const std::map<std::string, node_map> &mp,
                          const char *pattern) {
