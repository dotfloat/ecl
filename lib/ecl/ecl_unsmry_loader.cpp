...
unsmry_loader::unsmry_loader(const ecl_smspec_type *smspec,
    ...
    if (ecl_file_get_num_named_kw(file, PARAMS_KW) !=
        ecl_file_get_num_named_kw(file, MINISTEP_KW)) {
        ecl_file_close(file);
        throw std::bad_alloc();
    }
...
unsmry_loader::~unsmry_loader() { ecl_file_close(file); }
...
std::vector<double> unsmry_loader::get_vector(int pos) const {
    ...
    for (int index = 0; index < this->length(); index++) {
        ecl_file_view_index_fload_kw(file_view, PARAMS_KW, index, index_map,
                                     buffer);
...
std::vector<int> unsmry_loader::report_steps(int offset) const {
    ...
    for (int i = 0; i < ecl_file_view_get_size(this->file_view); i++) {
        const ecl_file_kw_type *file_kw =
            ecl_file_view_iget_file_kw(this->file_view, i);
...
std::vector<time_t> unsmry_loader::sim_time() const {
    if (this->time_index >= 0) {
        ...
        for (size_t i = 0; i < st.size(); i++)
            st[i] = ecl_util_make_date(util_round(day[i]), util_round(month[i]),
                                       util_round(year[i]));
